...
## 现状
- **版本**：v1.6.13（versionCode 38）· 包名 `com.brainquest.game` · Android 单机 APK + 应用内增量更新
...
- **v1.6.12**：云存档导入保留本机身份码（修复身份被存档覆盖的丢失 bug）；联机版本门控（非最新版拒绝对战+提示更新）；三小游戏操作按钮上移；五子棋双模式（人机三档 / 好友同机对坐：白方条倒置、轮次高亮、不结算奖励）
- **v1.6.13**：导航修复（Tab 去 restoreState，速算英雄→闯关后点大厅能回大厅）；五子棋体验（每次进入先到模式选择页、返回保留上局棋盘+「回到上一局」、棋盘垂直居中）；华容道一次手势只滑一步 + 底边框「门式出口」；云存档独立页（设置只留入口、身份码📋复制、口令改上传/下载时弹窗输入且每次打开清空防残留）
- **v1.6.11**：远程对战去除「甲方（房主）/乙方（挑战者）」旧称谓（服务器中立后无房主）→ 匹配页标「你 / 对手（先匹配）」，局域网保留「房主/挑战者」；大厅与排行榜的「房主出题」说明改为「按先匹配者/创建者」
