...
## 现状
- **版本**：v1.6.7（versionCode 32）· 包名 `com.brainquest.game` · Android 单机 APK + 应用内增量更新
...
| 云存档 | 存档码 + 口令（v1.6.7 起 PBKDF2→AES-GCM 加密后上传，服务器只存密文；旧 8 位明文码仍可下载）；`save_del` 删除通道；`save_get` 限流 |
...
- **v1.6.7**：云存档身份校验（口令 PBKDF2→AES-GCM 信封加密、`save_get` 限流、存档码 8→10 位、`save_del` 删除通道）+ 大学五科热更题包 165 题（`*_p3`）+ 上架技术准备（应用内隐私政策、明文流量白名单、主副名「脑力大冒险 / 学海星槎」、商店图标导出、README 截图补齐）。**注意：公网服务器尚未部署 v1.6.7 服务器代码**（需 `PK_SSH_PASS`），详见移交 03。
...
2. **部署 v1.6.7 服务器代码**（云存档加密校验/限流在 `update-server/pk_server.py` 已写好并过冒烟，等 `PK_SSH_PASS` 部署；新旧版本互相兼容，见移交 03）
